...
> 快照：2026-07-28。仓库现有 28 份视频 Manifest／123 张视频关键帧、1 份静态 Manifest／1 张论坛一手原图，以及 84 张独立 PNG 裁图，覆盖 44 个机器可读图标／状态键。正式清单以 [`catalog.json`](../../research/evidence/icons/catalog.json) 为准；`TEST-B` 与 `PIONEER-A` 严格隔离，全部样本仍需在国服重采。
...
84 张裁图全部未缩放、未抠透明底、未用生成式补图，也没有借用其他 RO 产品。校验器会重新读取父图，按 `x/y/width/height` 裁切并逐像素比对；父图哈希、裁图哈希、尺寸或坐标任一不一致都会失败。

当前置信度分布：81 张 `direct_clear`、3 张 `contextual_candidate`。样本类型为 12 张图标、56 张状态指示、11 张状态行、5 张弹窗／提示；构建分布为 75 张 `TEST-B`、9 张 `PIONEER-A`。
...
| `guild_caravan_batch` | 1 | 刷新为 `2/4` | 全部四批终局 |
...
该链适合离线验证“等待业务变化”，但提交涉及材料消耗，技术闭环不等于动作获授权。
...
同一耗尽直播又保留了[奖励通知父帧](../../research/evidence/video/twitch-vod-2804807809/frames/24420.00-farming-boost-exhausted-reward-feed.jpg)与 `farming_reward_feed` 裁图：页面保持 `0秒/180分` 时，左侧通知在连续九秒内新增齿轮、`Base经验 X3` 和物品 `X1`，背景伤害仍在发生。这能识别“耗尽但奖励通知仍活动”，不能把未显示的阶段硬命名为低倍，也不能推断掉率或疲劳。
...
以下内容能在 123 张视频父帧中定位，但没有进入 44 键／84 图的正式裁图库：
...
2026-07-28 又逐张查看巴哈五个重点主题的 39 个原图引用（包含跨帖重复图）。这些图只增加宠物、精炼、商城、普通战斗、主线对话、外观和座谈语境；没有画面支持“卡片／灵魂残响接近满载”、疲劳状态、真正满包、野外死亡或公会四批终局，因此该轮零图入库。当前第 84 张裁图来自 Twitch 耗尽页同步奖励通知，不用无关原图扩充数字。
...
- 公会商队：已有库存不足 `0/6` 行和购买／求助入口父帧；仍缺选中不足项后的禁用／失败、购买价格与结果、求助结果、四批终局；每日 `1/1 已完成`已有裁图但与四批关系未知；
...
在上述状态补齐前，84 张图片适合做离线分类、差异比较和国服采样种子；它们不能独立授权真实客户端动作。
