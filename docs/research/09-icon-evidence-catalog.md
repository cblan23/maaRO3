...
> 快照：2026-07-28。仓库现有 31 份视频 Manifest／138 张视频关键帧、1 份静态 Manifest／1 张论坛一手原图，以及 99 张独立 PNG 裁图，覆盖 52 个机器可读图标／状态键。正式清单以 [`catalog.json`](../../research/evidence/icons/catalog.json) 为准；`TEST-B` 与 `PIONEER-A` 严格隔离，全部样本仍需在国服重采。
...
以下内容能在 138 张视频父帧中定位，但没有进入 52 键／99 图的正式裁图库：
